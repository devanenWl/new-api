...
type SunoDataResponse struct {
	...
	Status     string          `json:"status" gorm:"type:varchar(20);index"` // 任务Status, submitted, queueing, processing, success, failed
...
type TaskDto struct {
	...
	Action     string          `json:"action"`  // 任务Type, song, lyrics, description-mode
	Status     string          `json:"status"`  // 任务Status, submitted, queueing, processing, success, failed
	FailReason string          `json:"fail_reason"`
