...
func authHelper(c *gin.Context, minRole int) {
		...
		if user != nil && user.Username != "" {
			if !validUserInfo(user.Username, user.Role) {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": "None权进行此Operation，Users信息None效",
				})
				c.Abort()
				return
			}
	...
	if !useAccessToken {
		...
		if apiUserIdStr == "" {
			c.JSON(http.StatusUnauthorized, gin.H{
				"success": false,
				"message": "None权进行此Operation，请Refresh页面或清空缓存后Retry",
			})
			c.Abort()
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusUnauthorized, gin.H{
				"success": false,
				"message": "None权进行此Operation，Log in信息None效，请重新Log in",
			})
			c.Abort()
			return
		...
		if id != apiUserId {
			c.JSON(http.StatusUnauthorized, gin.H{
				"success": false,
				"message": "None权进行此Operation，与Log inUsers不匹配，请重新Log in",
			})
			c.Abort()
			return
		}
