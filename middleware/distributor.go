	...
	return func(c *gin.Context) {
		...
		if len(allowIpsMap) != 0 {
			...
			if _, ok := allowIpsMap[clientIp]; !ok {
				abortWithOpenAiMessage(c, http.StatusForbidden, "您的 IP 不在API Keys允许访问的列表中")
				return
			}
		}
		userId := c.GetInt("id")
		...
		if tokenGroup != "" {
			...
			if _, ok := common.GetUserUsableGroups(userGroup)[tokenGroup]; !ok {
				abortWithOpenAiMessage(c, http.StatusForbidden, fmt.Sprintf("API KeysGroup %s 已被Disable", tokenGroup))
				return
			}
		...
		if ok {
			...
			if modelLimitEnable {
				...
				if tokenModelLimit != nil {
					if _, ok := tokenModelLimit[modelRequest.Model]; !ok {
						abortWithOpenAiMessage(c, http.StatusForbidden, "该API KeysNone权访问Model "+modelRequest.Model)
						return
					}
				} else {
					// token model limit is empty, all models are not allowed
					abortWithOpenAiMessage(c, http.StatusForbidden, "该API KeysNone权访问任何Model")
					return
				}
			}
			...
			if shouldSelectChannel {
				...
				if err != nil {
					...
					// 如果错误，而且Channel为空，说明是没有可用Channel
					abortWithOpenAiMessage(c, http.StatusServiceUnavailable, message)
					return
				}
				if channel == nil {
					abortWithOpenAiMessage(c, http.StatusServiceUnavailable, fmt.Sprintf("当前Group %s 下对于Model %s No available channels（数据库一致性已被破坏）", userGroup, modelRequest.Model))
					return
				}
			}
		}
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.Contains(c.Request.URL.Path, "/mj/") {
		...
		if relayMode == relayconstant.RelayModeMidjourneyTaskFetch ||
			...
			if midjourneyModel == "" {
				if !success {
					abortWithMidjourneyMessage(c, http.StatusBadRequest, constant.MjErrorUnknown, "Invalid request, None法解析Model")
					return nil, false, fmt.Errorf("Invalid request, None法解析Model")
				} else {
