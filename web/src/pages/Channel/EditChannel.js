...
const EditChannel = (props) => {
  ...
  const fetchUpstreamModelList = async (name) => {
    ...
    if (!err) {
      ...
      showSuccess("获取Model列表成功");
    } else {
      showError('获取Model列表失败');
    }
    setLoading(false);
  }
  ...
  const addCustomModels = () => {
    ...
    // 使用逗号分隔字符串，然后去除每个Model name前后的空格
      ...
      >
        <Spin spinning={loading}>
          ...
          {inputs.type === 3 && (
            <>
              ...
              <Input
                ...
                name='azure_other'
                placeholder={
                  '请EnterDefault API Version，For example：2023-06-01-preview，该配置可以被实际的请求Query参数所覆盖'
                }
                onChange={(value) => {
          ...
          {inputs.type === 41 && (
            <>
              ...
              <TextArea
                name='other'
                placeholder={
                  '请Enter部署地区，For example：us-central1\n支持使用Model mapping格式\n' +
                  '{\n' +
                  '    "default": "us-central1",\n' +
                  '    "claude-3-5-sonnet-20240620": "europe-west1"\n' +
                  '}'
          ...
          {inputs.type === 1 && (
            <>
              <div style={{ marginTop: 10 }}>
                <Typography.Text strong>Group织：</Typography.Text>
              </div>
              <Input
                label='Group织，可选，不填则为DefaultGroup织'
                name='openai_organization'
                placeholder='请EnterGroup织org-xxx'
                onChange={(value) => {
          ...
          <div style={{ marginTop: 10, display: 'flex' }}>
            <Space>
              ...
              />
              <Typography.Text strong>
                是否自动Disable（仅当自动Disable开启时有效），Close后不会自动Disable该Channel：
              </Typography.Text>
            </Space>
          </div>
          ...
          {inputs.type === 22 && (
            <>
              ...
              <Input
                name='base_url'
                placeholder={
                  '请Enter私有部署地址，格式为：https://fastgpt.run/api/openapi'
                }
                onChange={(value) => {
                  handleInputChange('base_url', value);
                }}
          ...
          <TextArea
            placeholder={`此项可选，用于复写返回的Status码，比如将claudeChannel的400错误复写为500（用于Retry），请勿滥用该功能，For example：\n${JSON.stringify(STATUS_CODE_MAPPING_EXAMPLE, null, 2)}`}
