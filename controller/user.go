...
func Register(c *gin.Context) {
	...
	if err := model.DB.Where("username = ?", cleanUser.Username).First(&insertedUser).Error; err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "UsersSign up失败或UsersID获取失败",
		})
		return
	}
	// 生成Default key
	if constant.GenerateDefaultToken {
		...
		// 生成Default key
		token := model.Token{
			UserId:             insertedUser.Id, // 使用插入后的UsersID
			Name:               cleanUser.Username + "的初始API Keys",
			Key:                key,
